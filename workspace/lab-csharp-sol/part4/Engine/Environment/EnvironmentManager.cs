...
namespace Unibo.ISI.ProgramazioneAdOggetti.Esercitazioni
{
  class EnvironmentManager : BaseEnvironmentManager
  {
    ...
    protected override BaseEnvironmentManager.OrganismInfo TryAddOrganism(IOrganism organism, int x, int y)
    {
      ...
      return envRect.Contains(oi.BoundingBox) && IsRegionEmpty(oi.BoundingBox, null) ? oi : null;
    }
    ...
    public override void AdvanceTime()
    {
      ...
      foreach (var o in organisms)
      {
        ...
        if (!TryMove(o))
        {
          // non si può muovere: imposta la velocità a zero come richiesto nel punto 6) [Facoltativo]
          o.Speed = 0;
        }
      }
    }
    ...
    private bool TryMove(OrganismInfo o)
    {
      ...
      int dx = o.Direction == Direction.West ? -o.Speed : o.Direction == Direction.East ? o.Speed : 0;
      int dy = o.Direction == Direction.North ? -o.Speed : o.Direction == Direction.South ? o.Speed : 0;
      rect.Offset(dx, dy);
      if (envRect.Contains(rect) && IsRegionEmpty(rect, o))
      {
        o.X += dx;
        o.Y += dy;
        return true;
