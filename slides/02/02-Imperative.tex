...
\title[OOP02 -- Imperative]{02 \\ Uso avanzato degli strumenti di laboratorio, programmazione imperativa e costruttori}
...
\begin{itemize}
\item Abbiamo già visto come compilare file sorgenti Java (file \texttt{.java}), generando il bytecode (file \texttt{.class}) nella medesima directory 
...
\fr{Esempio 1: compilare di più file da e in qualunque punto}{
  ...
  \end{block}
  \begin{itemize}
    \item Si posizioni il terminale in \texttt{C:} (in \texttt{/} per chi ha Mac o Linux) e si riprovi a compilare \texttt{Calculator.java}
  \end{itemize}
  ...
  \begin{itemize}
    \item Solo su sistemi *nix si possono usare wildcard in più punti del path
    \item Ad esempio \texttt{progetti/*/src/*.java} elenca tutti i file con estensione java dentro ciascuna cartella \texttt{src} di ciascuna cartella dentro \texttt{progetti}
  \end{itemize}
}
...
\fr{Il classpath in dettaglio}{
  \iz{
    \item Il classpath è l'insieme delle directory in cui Java cerca classi e package
   ...
   \bl{In \cil{java}}{
    ...
    \iz{
      \item Uso di classi compilate fornite, di solito come librerie esterne in formato \texttt{jar} (le vedremo) 
    }
   }
  \bl{In \cil{javac}}{
...
\fr{Esempio 2: Compilazione ed esecuzione di classi strutturate in package}{
  \iz{
      ...
      \bx{\textbf{Rappresenta su filesystem la struttura del package \texttt{oop.lab02.math}!}}
...
\fr{Esempio 2: la classe \texttt{ComplexNum}}{
   ...
   \bx{\center{\textbf{\texttt{ComplexNum} è ora definita all'interno del package}}}
}
\fr{Esempio 2: compilazione/utilizzo di javac}{
  \en{
      \item Spostarsi (se non li si è già fatto) in \texttt{esempio2}
      \item Eseguire la compilazione:
      \iz {
   ...
   \bx{\center{\textbf{E per l'esecuzione?}}}
...
\fr{Esempio 2: esecuzione/uso di java}{
  \iz{
   ...
   \bx{\center{\textbf{No! La classe è infatti definita all'interno di un package}}}  
}
...
\fr{Esempio 2: esecuzione/uso di java}{
  \iz{
	...
	\item \texttt{java -cp bin ComplexNum}
      ...
      \item La JVM si aspetterebbe di trovare un file \texttt{ComplexNum.class} all'interno di \texttt{bin}
   }
   \bx{\center{\textbf{Qual è dunque il metodo corretto per eseguire con successo \texttt{ComplexNum}?}}}  
}
...
\fr{Esempio 3: compilazione}{
   \bl{Comprensione degli errori}{\sizedcode{\tiny}{code/esempio3/src/oop/lab02/math/error.txt}}
     \iz{
      \item Il compilatore si aspetta di trovare nel classpath il bytecode della classe \texttt{ComplexNum} (file \texttt{ComplexNum.class})
...
\fr{Esempio 3: secuzione di \texttt{UseComplex}}{
  \iz{
   ...
   Si ricorda che ``\texttt{..}'' significa ``livello directory superiore a quello corrente'', mentre ``\texttt{.}'' significa ``livello directory corrente''
}
...
\begin{frame}{Consiglio finale}
  ...
  \begin{block}{}
    ...
    \end{itemize}
  \end{block}
%   Vi verranno elencate molte opzioni (molte delle quali non sono coperte nel corso) assieme con le loro descrizioni. Se vi doveste dimenticare la sintassi di una opzione, questo è un modo per ricordarla.
	...
	\bl{Implementare i seguenti metodi statici nella classe WorkWithArrays}{
		\iz{
			\item \cil{public static int[] revertUpTo(int[] array,int stopEl)}
				\iz{
					\item Inverte gli elementi di \cil{array} dall'inizio fino all'elemento \cil{stopElem}. Gli elementi da \cil{stopEl} in poi devono rimanere invariati
					\item p.e. revertUpTo([1,2,3,4,5,6,7],5) $\rightarrow$ [4,3,2,1,5,6,7]
				}		
...
\end{document}
			...
			\item Un costruttore \cil{Train(int nFCReservedSeats, int nSCReservedSeats)}, che consenta di creare un nuovo oggetto di classe \cil{Train} con un certo numero di posti di prima e seconda classe già riservati
			\iz {
				\item I campi della classe dovranno essere inizializzati con dei valori di default (sensati) scelti dallo studente
				\item I campi della classe dovranno essere inizializzati con dei valori di default (sensati) scelti dallo studente
			}
...
\fr{Esempio 1: File sorCompilazione di tre packages in one-shot}{
  \iz{
      \item Aprire un terminale
      \item Posizionarsi nella directory in cui si è scompattato il codice di questo laboratorio
      \item posizionarsi nella directory \texttt{compilazione1}
      \item Creare una cartella \cil{bin}:comando \cil{mkdir bin}
      \item Eseguire la compilazione:
      \iz {
	\item Su sistemi UNIX (Linux, Mac OSX, BSD) è possibile eseguire il comando \\ \texttt{javac -d bin src/oop/lab05/packages/pkg*/*}
	\item Su sistemi Windows, è invece necessario eseguire (\textbf{unico comando}) \\ \texttt{javac -d bin src/oop/lab05/packages/pkg1/* src/oop/lab05/packages/pkg2/* src/oop/lab05/packages/pkg3/* }
	\item Motivo: l'espansione della wildcard ``*'' è demandata al sistema operativo.
      }
      \item Verificare la presenza dei file \texttt{.class} all'interno della directory \texttt{bin}, divisi correttamente in cartelle che ricostruiscono la struttura dei package.
