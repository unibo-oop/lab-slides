...
\fr{Debugging: Test delle Principali Operazioni}{
	\tiny Classe d'esempio \cil{it.unibo.oop.lab04.bank.DebugTestIBankAccount}\normalsize
   \sizedcode{\tiny}{code/Code/src/it/unibo/oop/lab04/bank/DebugTestIBankAccount.java}
}
...
\fr{History delle Modifiche 1/2}{
	
	\bl{Aiuto! Ho erroneamente cancellato/modificato parti del mio sorgente!}{
		\iz{
			\item(Potenzialmente) nulla è perduto!
				\iz{ 
					\item	Eclipse gestisce automaticamente un supporto (di base) di \textcolor{blue}{versioning} dei progetti all'interno del vostro workspace
					
					\item Possiamo quindi cercare di recuperare le modifiche perse analizzando versioni precedenti dei sorgente salvate nella \textcolor{blue}{local history} di Eclipse
				}
			\item La history gestita da Eclipse è completamente configurabile	
			\iz{ \item p.e. dimensione massima su disco della history, numero massimo di versioni salvate per un dato file, etc. }
		}
	}
	\bl{Nota importante}{
		\iz{
			\item La history delle modifiche è un supporto \textit{``comodo e utile'' }
			\item \alert{Non è però da considerare come una valida alternativa ai sistemi di versioning al supporto dello sviluppo di software (che vedremo)}
			\iz{ \item p.e. modfiche potrebbero essere perse una volta raggiunta la dimensione massima della history su disco}
		}
	}
}

\fr{History delle Modifiche 2/2}{
	\fg{height=0.8\textheight}{img/local_history1.png}
}
