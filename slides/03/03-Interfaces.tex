...
\fr{Preparazione Ambiente di Lavoro 1/2}{
	\iz{
		...
		\item Scaricare dalla sezione \texttt{lab} del sito il file \texttt{oop1415-lab03.zip} contenente il materiale dell'esercitazione odierna
...
\fr{Preparazione Ambiente di Lavoro 2/2}{
	\iz{
		...
		\item Importare il progetto \texttt{OOP1415-LAB} con la procedura di importazione dei progetti mostrata nelle slide precedenti
...
\fr{Applichiamo il Principio di Incapsulamento 1/2}{
	\bl{Analizzare la classe \cil{oop1415.lab03.encapsulation.Student}}{
		\iz{
			\item E' una versione modificata della classe definita nel package \cil{oop1415.lab02.constructors} e vista la settimana scorsa
...
\fr{Applichiamo il Principio di Incapsulamento 2/2}{
	\bl{}{
		Modificare le classi Calculator e Smartphone contenute nel package \cil{oop1415.lab03.encapsulation} applicando il principio di incapsulamento
	}
	...
	\bl{Nota}{
		Una volta vista l'ereditarietà (estensione di classi esistenti, meccanismo introdotto nelle prossimi lezioni) vedremo/discuteremo in dettaglio quando è il caso di non dichiarare dei campi privati
	}
}

\section{Esercizi Base da Svolgere in Autonomia}

\fr{\cil{oop1415.lab03.encapsulation.AccountHolder}}{
	\bl{}{
		\iz{
	...
	\bl{}{
		Implementare il metodo \cil{String toString()} lasciato incompleto
			\iz{
				\item Metodo visto a lezione, per convenzione Java deve restituire una rappresentazione in stringa dell'oggetto
...
\fr{\cil{oop1415.lab03.encapsulation.BankAccount} 1/2}{
...
%		  \item \cil{END_OF_MONTH_FEE // Costante relativa alle spese di gestione di fine mese}
	...
	\bl{Costruttori}{
		\iz{
			\item \cil{public BankAccount(int usrID, double initAmount)}
		}
	}
}

\fr{\cil{oop1415.lab03.encapsulation.BankAccount} 2/2}{
...
\fr{Test di BankAccount}{
	\bl{}{
		Effettuare il testing della classe \cil{BankAccount} appena implementata seguendo le indicazioni fornite nei commenti in \cil{oop1415.lab03.encapsulation.TestBankAccount} 
	}
}

\fr{\cil{oop1415.lab03.interfaces.IBankAccount} 1/2}{
	\bl{}{
		Modificare la classe \cil{BankAccount} in modo da implementare l'interfaccia \cil{IBankAccount}
                \iz{ \item realizzare solamente i metodi necessari alla corretta implementazione dell'interfaccia da parte di \cil{BankAccount}}
	}
	\bl{\cil{oop1415.lab03.interfaces.IBankAccount}}{
		\sizedcode{\scriptsize}{code/IBankAccount.java}
	}
}	

\fr{\cil{oop1415.lab03.interfaces.IBankAccount} 2/2}{
...
\fr{\cil{oop1415.lab03.interfaces.StrictBankAccount}}{	
	\bl{}{Implementare in autonomia la classe \cil{StrictBankAccount}}
	\bl{Linee guida}{
		\iz{
			\item La classe dovrà implementare l'interfaccia \cil{IBankAccount}
...
\fr{\cil{BankAccount e StrictBankAccount: Test}}{	
		\bl{}{
			Effettuare il testing delle classi \cil{BankAccount} e \cil{StrictBankAccount} appena implementate seguendo le indicazioni fornite nei commenti in \cil{oop1415.lab03.interfaces.TestIBankAccount} 
		}
}
...
\fr{Modelliamo gli Utenti: Interfaccia IUser}{
	\bl{}{
		\iz{
			\item \cil{IUser} definisce l'interfaccia di un generico utente di ACME-Exams
			\item Analizzare l'interfaccia \cil{IUser} e la classe \cil{Student} che la implementa
		}
	}
	\bl{\cil{oop1415.lab03.acme.IUser}}{
		\sizedcode{\scriptsize}{code/IUser.java}
	}
}

\fr{\cil{oop1415.lab03.acme.Professor}}{
	\bl{Implementare la classe \cil{Professor} (dovrà implementare \cil{IUser})}{
...
\fr{Forme di Dipendenza tra Classi: Composizione 1/2}{
	\bl{Implementare la classe \cil{oop1415.lab03.acme.Exam}}{
...
\fr{\cil{oop1415.lab03.acme.Testing}}{
	\bl{}{
...
\fr{IShape: Un' Interfaccia per Modellare Figure Geometriche}{
        \bl{\cil{oop1415.lab03.interfaces.IShape.java}}{Realizzare \textbf{da zero} un'interfaccia che consenta di calcolare \textbf{area} e \textbf{perimetro} di una qualsivoglia \textbf{figura geometrica}}
	%\code{code/IShape.java}
...
\fr{\cil{oop1415.lab03.shapes.Circle}}{
	\bx{Realizzare da zero la classe \cil{Circle} che, implementando l'interfaccia \cil{IShape}, consenta di definire un cerchio
		\iz{	
...
\fr{Estensione per Figure con Lati Limitati}{
	...
	\bl{Realizzare \cil{oop1415.lab03.interfaces.IShapeWithLimitedEdges} che estende \cil{IShape}}{
		\iz{
			\item definire il metodo \cil{int getNumEdges()} che restituisce il numero di lati della forma geometrica
			\item I metodi definiti in \cil{IShape} vengono perciò \textit{ereditati}
			\item Pertanto, le classi che implementeranno \cil{IShapeWithLimitedEdges} dovranno obbligatoriamente definire 3 metodi relativi a questa interfaccia
		}
	}	
}
...
\fr{Square, Rectangle and ScaleneTriangle}{
	\bl{Implementare le classi \cil{Square}, \cil{Rectangle} e \cil{ScaleneTriangle}}{
		\iz{
			\item Le tre classi dovranno implementare \cil{IShapeWithLimitedEdges}
