...
\fr{Il Laboratorio di OOP}{
  ...
  \bl{Titolare del corso di laboratorio: Ing. Danilo Pianini}{
    \iz{ 
      \item e-mail | \texttt{danilo.pianini@unibo.it}
      \item homepage | \texttt{http://danilopianini.apice.unibo.it}
      \item orario di ricevimento | Martedì, dalle ore 17:00
    }
  }
  \bl{Tutors: Ing. Angelo Croatti \& Ing. Giovanni Ciatto}{
    \iz{ 
      \item e-mail | \texttt{a.croatti@unibo.it} \& \texttt{giovanni.ciatto@unibo.it} 
    }
  }
}
...
\iz{ 
\item Java Overview (linguaggio + piattaforma)
...
%====================
\begin{frame}[fragile, allowframebreaks]{Java nel mondo ``reale''}
	\fg{height=0.80\textheight}{img/javaTrend.png}
	\fg{height=0.80\textheight}{img/javaTrend_highlight.png}
	\fg{height=0.80\textheight}{img/tiobe2016}
\end{frame}
...
\fr{Java Development Kit (JDK)}{
	...
	\bl{Noi faremo riferimento al JDK}{Include il necessario per eseguire applicazioni	Java (JRE), i tool (e librerie) per sviluppare applicazioni e la relativa  documentazione}
...
\fr{Preparazione Ambiente di Lavoro 1/3}{
	\iz{
		...
		\iz{
			\item \textcolor{blue}{\url{https://elearning-cds.unibo.it/course/view.php?id=11689}}
		}
...
\fr{Modalità di Lavoro}{
	\bl{}{
		\en{
			...
			\iz {
				\item \textbf{Si prega di non modificare, quando esplicitamente forniti, i nomi dei metodi, né i loro tipi in ingresso o di ritorno}
			}
...
\fr{Costruzione di Semplici Classi 1/5}{
	\bl{}{
...
\fr{Costruzione di Semplici Classi 2/5 (A)}{
	\bl{}{
...
\fr{Costruzione di Semplici Classi 2/5 (B)}{
	\bl{Operazioni sui numeri complessi --- Breve ripasso}{
		Siano $z,w \in \mathbb{C} : z = a + ib,\ w = c + id$, allora:
		\iz {
			\item Confronto: $z = w \iff a = b \wedge c = d$
			\item Somma algebrica: $z \pm w = (a \pm c) + i(b \pm d)$
			\item Prodotto: $z \cdot w = (ac - bd) + i(bc + ad)$
			\item Rapporto: $\frac{z}{w} = \frac{ac + bd}{c^2 + d^2} + i\frac{bc - ad}{c^2 + d^2}$
		}
	}	
}

\fr{Costruzione di Semplici Classi 3/5 (A)}{
	\bl{}{
	...
	\bl{Caratteristiche della classe \cil{Calculator}:}{
		\iz{
			\item Campi: - (nessuno) (il che la rende una classe ``degenere'' dal punto di vista dell'OOP)
			\item Metodi (oltre al \texttt{main})
			\iz{
				\item \cil{add/sub/mul/div}
					\iz{
...
\fr{Costruzione di Semplici Classi 3/5 (B)}{
...
\fr{Costruzione di Semplici Classi 4/5}{
...
\fr{Costruzione di Semplici Classi 5/5 (A)}{
	\bl{}{
		Implementare una classe Java che modelli il concetto di treno
	}
\bl{Caratteristiche della classe \cil{Train} - Campi}{ 
\iz{
\item \cil{int nTotSeats} (n. posti totali del treno)
\item \cil{int nFirstClassSeats} n. posti in prima classe
\item \cil{int nSecondClassSeats} n. posti in seconda classe
\item \cil{int nFirstClassReservedSeats} n. di posti prenotati in prima classe
\item \cil{int nSecondClassClassReservedSeats} n. di posti prenotati in seconda classe
}
}
}

\fr{Costruzione di Semplici Classi 5/5 (B)}{
\bl{Caratteristiche della classe \cil{Train} - Metodi}{ 
\iz{
\item \cil{build} per l'inizializzazione opportuna dei campi. Nota: valutare quale sia l'input minimo necessario per il metodo build.)

\item \cil{reserveFirstClassSeats}
				\iz{
					\item Input: \cil{int nSeats}
				}
\item \cil{reserveSecondClassSeats}
				\iz{
					\item Input: \cil{int nSeats}
				}				
				...
				\item \cil{getFirstClassOccupancyRatio}
				\iz{
					\item Returns: \cil{double} (percentuale posti occupati in prima classe)
				}
				
								\item  \cil{getSecondClassOccupancyRatio}
				\iz{
					\item Returns: \cil{double} (percentuale posti occupati in seconda classe)
				}
...
\fr{Costruzioni di Semplici Classi 5/5 (C)}{
	\bl{Testing}{
		\iz{
...
%		Java gestisce in maniera diversa da C il passaggio dei parametri
%		\iz{
%			\item In particolare
%			\iz{
