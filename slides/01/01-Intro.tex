...
\title[OOP01 -- Intro]{01 \\ Introduzione al Laboratorio \& Tool di Base}
...
\fr{Java Development Kit (JDK)}{
	...
	\bl{Esistono diverse distribuzioni di Java}{
		\begin{description}
			\item[JRE] -- Java Runtime Environment: mera esecuzione di programmi
			\item[JDK] -- JRE + tool per lo sviluppo di programmi
	...
	\bl{I Principali Tool}{
		\begin{description}
			...
			\item[jar] -- Per compressione di file (bytecode e risorse) in un archivio eseguibile
		\end{description}
	}
}
...
%====================
%Compilazione \& Esecuzione da Riga di Comando
%====================
\section{Compilazione \& Esecuzione da Riga di Comando}


\fr{Compilazione \& Esecuzione (base) 1/2}{
	\iz{
		\item	Aprire il programma notepad++
			\iz{
					\item Nostro IDE per questa prima lezione
			}
		\item Da notepad++ aprire la classe \texttt{HelloWorld.java}
		\item Studiare brevemente il comportamento della classe prima di procedere con i passi successivi
	}	
	\fg{height=0.6\textheight}{img/notepad.png}
}


\fr{Compilazione \& Esecuzione (base) 2/2}{
...
\section{Sviluppo \& Test di Semplici Classi}
...
\fr{Un Riconoscitore della Sequenza Ripetuta 1234}{
	...
	\bl{Traccia Risolutiva}{
		\iz{
			...
			\item Tenere traccia all'interno della classe delle informazioni di stato ritenute necessarie (p.e. prossimo carattere da dover essere riconosciuto)
