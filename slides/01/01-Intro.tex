...
\begin{frame}[fragile]{Come funziona il laboratorio di OOP}
	\begin{block}{Organizzazione}
		\begin{itemize}
			...
			\item Esercizi ``aggiuntivi'' (che non sono opzionali)
		\end{itemize}
	\end{block}
	\begin{block}{Come svolgere gli esercizi}
		\begin{itemize}
			\item Provate da soli. Se bloccati per qualche minuto, chiamate il docente
			\item Fate riferimento alle slides del corso, tenetele con voi. Specialmente all'inizio, \textbf{evitate} di andare a prendere pezzi di codice online, assicuratevi di avere sempre chiaro cosa stiate facendo e perché
			\item Una volta terminato, chiamate il docente per la correzione
			\item Riuscire a completare anche gli esercizi ``aggiuntivi'' nelle quattro ore è indice di buona preparazione. Non riuscirci non è preoccupante, ma raccomandiamo caldamente di terminarli a casa \textbf{prima} del laboratorio successivo.
		\end{itemize}
	\end{block}
\end{frame}
...
\fr{Preparazione Ambiente di Lavoro 1/3}{
	\iz{
		...
		\iz{
			\item \textcolor{blue}{\url{bit.ly/oop2016cesena}}
		}
